...
static void on_write(ble_lights_t * p_lights, ble_evt_t * p_ble_evt)
{
    ...
    if (p_lights->is_notification_supported)
    {
        ...
        //write event for light_level low value char.   
        if (
        ...
        //write event for light_level high value char. 
        ...
        //write event for light_level alarm set

        if (
                (p_evt_write->handle == p_lights->light_alarm_set_handles.cccd_handle)
                &&
                (p_evt_write->len == 2)
                )
        {
        ...
        //write event for light_level alarm

        if (
                (p_evt_write->handle == p_lights->light_alarm_handles.cccd_handle)
                &&
                (p_evt_write->len == 2)
                )
        {
        ...
        //write event for current light level
            ...
            )
    {
        ...
        p_lights->light_low_level[0] =   p_evt_write->data[0];
        p_lights->light_low_level[1] =   p_evt_write->data[1];
    ...
    {
        // update the light service structure
        p_lights->light_high_level[0] =   p_evt_write->data[0];
        p_lights->light_high_level[1] =   p_evt_write->data[1];
    ...
    if (
            (p_evt_write->handle == p_lights->light_alarm_set_handles.value_handle) 
            && 
            (p_evt_write->len == 1)
            )
    {
...
static uint32_t current_light_level_char_add(ble_lights_t * p_lights, const ble_lights_init_t * p_lights_init)
{
    ...
    ble_uuid.uuid = GROW_PROFILE_LIGHTS_CURR_LIGHT_CHAR_UUID;
...
static uint32_t light_low_value_char_add(ble_lights_t * p_lights, const ble_lights_init_t * p_lights_init)
{
    ...
    // Add custom UUID to the characteristic 
    ble_uuid.type = p_lights->uuid_type;
    ble_uuid.uuid = GROW_PROFILE_LIGHTS_LIGHT_LOW_CHAR_UUID;  
    ...
    climate_light_low_value[1] = p_lights_init->light_low_value[1];
    climate_light_low_value[0] = p_lights_init->light_low_value[0];		
    memset(&attr_char_value, 0, sizeof(attr_char_value));
...
static uint32_t light_high_value_char_add(ble_lights_t * p_lights, const ble_lights_init_t * p_lights_init)
{
    ...
    // Add custom UUID to the characteristic 
    ble_uuid.type = p_lights->uuid_type;
    ble_uuid.uuid = GROW_PROFILE_LIGHTS_LIGHT_HIGH_CHAR_UUID;
    ...
    climate_light_high_level[1] = p_lights_init->light_high_value[1];  /*Default values for light_level high value*/
    climate_light_high_level[0] = p_lights_init->light_high_value[0];	
...
static uint32_t light_alarm_set_char_add(ble_lights_t * p_lights, const ble_lights_init_t * p_lights_init)
{
    ...
    // Add custom UUID to the characteristic 
    ble_uuid.type = p_lights->uuid_type;
    ble_uuid.uuid = GROW_PROFILE_LIGHTS_LIGHT_ALARM_SET_CHAR_UUID;  
    ...
    climate_light_alarm_set = p_lights_init->light_alarm_set;
    ...
    err_code = sd_ble_gatts_characteristic_add(p_lights->service_handle, &char_md,
    &attr_char_value,
    &p_lights->light_alarm_set_handles);
    if (err_code != NRF_SUCCESS)
    {
        return err_code;
    }
...
static uint32_t light_alarm_char_add(ble_lights_t * p_lights, const ble_lights_init_t * p_lights_init)
{
    ...
    // Add custom UUID to the characteristic 
    ble_uuid.type = p_lights->uuid_type;
    ble_uuid.uuid = GROW_PROFILE_LIGHTS_LIGHT_ALARM_CHAR_UUID;  
...
uint32_t ble_lights_init(ble_lights_t * p_lights, const ble_lights_init_t * p_lights_init)
{
    ...
    p_lights->light_low_level[0]     = p_lights_init->light_low_value[0];
    p_lights->light_low_level[1]     = p_lights_init->light_low_value[1];
...
uint32_t ble_lights_level_alarm_check(ble_lights_t * p_lights)
{
    ...
    light_level_low_value 						= 	(p_lights->light_low_level[0])<<8;   /*convert the 8 bit arrays to a 16 bit data*/
    light_level_low_value 						= 	light_level_low_value | (p_lights->light_low_level[1]);	

    // Get the light_level high value set by the user from the service 

    light_level_high_value            = 	(p_lights->light_high_level[0])<<8;	  /*convert the 8 bit arrays to a 16 bit data*/
    light_level_high_value 			 			= 	light_level_high_value | (p_lights->light_high_level[1]);


    // Check whether the light_level is out of range if alarm is set by user 		
    if(p_lights->light_alarm_set != 0x00)
    {
    ...
    {	
        ...
        if ((p_lights->conn_handle != BLE_CONN_HANDLE_INVALID) && p_lights->is_notification_supported)
        {
            ...
            hvx_params.handle   = p_lights->light_alarm_handles.value_handle;
            hvx_params.type     = BLE_GATT_HVX_NOTIFICATION;
            hvx_params.offset   = 0;
