...
*
* @param[in]   p_ble_evt   Blue tooth stack event.
*/
...
void do_temperature_measurement(uint8_t *buffer) 
{
    ...
    current_temperature = get_tmp102_oneshot_temp();        /* Read temperature from tmp102 in one shot mode*/
...
void do_soil_mois_measurement(uint8_t *soil_mois_level) 
{  


    *soil_mois_level    = do_soil_moisture_measurement();   /* Read soil moisture level from ADC*/
...
void broadcast_mode(void)
{
    ...
    if(BROADCAST_MODE)											/* Broadcast mode is set True from connect.c start broadcasting*/
    {	
        ble_stack_init();
