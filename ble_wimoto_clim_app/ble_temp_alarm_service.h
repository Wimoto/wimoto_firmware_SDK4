...
typedef struct
{
    ...
    uint8_t                       climate_temperature_low_level[2];  /**< temperature low level */
    uint8_t                       climate_temperature_high_level[2]; /**< temperature high level */
    uint8_t												climate_temperature_alarm_set;     /**< Alarm set for temperature **/
    uint8_t												climate_temperature_alarm;   			 /**< Alarm for temperature **/
...
{                                                                    
    ...
    ble_gatts_char_handles_t      climate_temp_alarm_set_handles;  	 /**< Handles for temperature alarm set characteristic. */
    ble_gatts_char_handles_t      climate_temp_alarm_handles;      	 /**< Handles for temperature alarm characteristic. */
    ble_gatts_char_handles_t      switch_mode_handles;      	       /**< Handles for temperature alarm characteristic. */
    uint16_t                      report_ref_handle;              	 /**< Handle of the Report Reference descriptor. */
    uint8_t                       climate_temperature_low_level[2];  /**< temperature low level for alarm for temperature Service. */
    uint8_t                       climate_temperature_high_level[2]; /**< temperature high level for alarm for temperature Service. */
    uint8_t												climate_temperature_alarm_set;   	 /** Alarm set for temperature **/
    uint8_t												climate_temperature_alarm;   			 /** Alarm for temperature **/
...
*
* @param[out]  p_bas       Temperature Service structure. This structure will have to be supplied by
...
*/
uint32_t ble_temps_init(ble_temps_t * p_bas, const ble_temps_init_t * p_temps_low_init);
...
*       client.
*
* @param[in]   p_bas      Temperature Service structure.
* @param[in]   p_ble_evt  Event received from the BLE stack.
*/
void ble_temps_on_ble_evt(ble_temps_t * p_bas, ble_evt_t * p_ble_evt);
...
*/
uint32_t ble_temps_level_alarm_check(ble_temps_t *);
